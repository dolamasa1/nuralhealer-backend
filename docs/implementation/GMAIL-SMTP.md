...
- **Reliability**:
    - **Startup Catch-up**: Processes any backlog immediately upon connection.
    - **Event-Scheduled Retries**: Failed jobs effectively managed via `ScheduledExecutorService` (30s, 60s, 120s backoff).
- **Concurrency**:
    - **Atomic Locking**: Uses native SQL to lock jobs (`status='processing'`) preventing race conditions.
    - **Isolation**: Each job runs in its own transaction template.
