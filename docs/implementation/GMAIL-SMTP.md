...
- **Frequency**: Every 15 seconds (`@Scheduled(fixedRate = 15000)`).
- **Batching**: Processes up to 50 jobs per cycle.
- **Retry Logic**: 3 attempts before marking a job as `failed`.
...
- **ID Fallback**: If a job payload is missing `recipientEmail`, the processor uses the `userId` to look up the user's primary email from the `users` table.
...
## 5. Notification System Integration (✅ SIMPLIFIED)

### 5.1 Deep Integration Strategy
The email system is **directly connected** to the notification engine. The `notifications` table acts as the source of truth, and Gmail acts as the asynchronous delivery channel.
...
3. **Queue**: Database trigger automatically inserts a job into `message_queues`.
4. **Process**: `EmailQueueProcessor` polls every 15s.
5. **Send**: Gmail SMTP delivers the HTML template.
...
4. Wait 15 seconds for processor to run
5. Verify email received with welcome template
6. Check `notifications.delivery_status->>'email'` = `true`
