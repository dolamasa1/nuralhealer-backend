-- ================================================================
-- NEURALHEALER - COMPLETE DATABASE SCHEMA (Spring Boot Compatible)
-- ================================================================

-- Removed CREATE DATABASE and extensions for Spring Boot init
...
CREATE TABLE IF NOT EXISTS engagement_access_rules (
...
CREATE TABLE IF NOT EXISTS engagement_verification_tokens (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS active_service_subscriptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS security_authentication_tokens (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS user_management_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  metric_name VARCHAR(100) NOT NULL,
  metric_value DECIMAL,
...
-- Engagement indexes
CREATE INDEX IF NOT EXISTS idx_engagements_doctor_id ON engagements(doctor_id);;
CREATE INDEX IF NOT EXISTS idx_engagements_patient_id ON engagements(patient_id);;
CREATE INDEX IF NOT EXISTS idx_engagements_status ON engagements(status);;
CREATE INDEX IF NOT EXISTS idx_engagements_dates ON engagements(start_at, end_at);;
...
CREATE INDEX IF NOT EXISTS idx_doctor_patients_is_active ON doctor_patients(is_active);;
CREATE INDEX IF NOT EXISTS idx_doctor_patients_current_engagement ON doctor_patients(current_engagement_id);;

-- Message indexes
CREATE INDEX IF NOT EXISTS idx_engagement_messages_engagement_id ON engagement_messages(engagement_id);;
CREATE INDEX IF NOT EXISTS idx_engagement_messages_sender ON engagement_messages(sender_id);;
CREATE INDEX IF NOT EXISTS idx_engagement_messages_created_at ON engagement_messages(created_at);;
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_patient_started ON ai_chat_sessions(patient_id, started_at);;
...
CREATE INDEX IF NOT EXISTS idx_auth_tokens_user_id ON security_authentication_tokens(user_id);;
CREATE INDEX IF NOT EXISTS idx_auth_tokens_expires_at ON security_authentication_tokens(expires_at);;
...
CREATE INDEX IF NOT EXISTS idx_engagements_doctor_patient_status 
...
CREATE OR REPLACE TRIGGER update_doctor_profiles_updated_at BEFORE UPDATE ON doctor_profiles
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_patient_profiles_updated_at BEFORE UPDATE ON patient_profiles
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_engagements_updated_at BEFORE UPDATE ON engagements
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_doctor_patients_updated_at BEFORE UPDATE ON doctor_patients
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_engagement_access_rules_updated_at BEFORE UPDATE ON engagement_access_rules
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_system_settings_updated_at BEFORE UPDATE ON system_settings
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_ai_chat_sessions_updated_at BEFORE UPDATE ON ai_chat_sessions
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;

CREATE OR REPLACE TRIGGER update_active_service_subscriptions_updated_at BEFORE UPDATE ON active_service_subscriptions
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();;
...
CREATE SEQUENCE IF NOT EXISTS engagement_id_seq;;
CREATE OR REPLACE TRIGGER set_engagement_id BEFORE INSERT ON engagements
...
CREATE OR REPLACE TRIGGER engagement_status_change
...
CREATE OR REPLACE TRIGGER relationship_access_change
